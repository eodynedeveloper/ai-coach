import requests 
import datetime 
import time 

headers = {'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..J5tphIDwZf_2DYJc6YhTl2_AGRowj0b2lbvMkfI2X7c'}

class Endpoints:
    def __init__(self):
        self.get_history_url = "https://rgsweb.eodyne.com/rgsmims/backend/development/webservices/src/ai-coach/get-patient-messages-and-sessions"
        self.get_time_slot_url = "https://rgsweb.eodyne.com/rgsmims/backend/development/webservices/src/app/1/coach-messages/get-training-time"
        self.schedule_notif_url = "https://rgsweb.eodyne.com/rgsmims/backend/development/webservices/src/ai-coach/add-coach-message"
        self.get_language_url = "https://rgsweb.eodyne.com/rgsmims/backend/development/webservices/src/app/1/patient-language/get"
    
    def get_history(self):
        response = requests.post(self.get_history_url, headers=headers)
        return (response.json())
    
    def get_language(self, patient_id):
        response = requests.get(f"{self.get_language_url}/{patient_id}", headers=headers)
        try:
            return (response.json())
        except:
            return {"LANGUAGE_KEY": "English"}
    
    def get_time_slot(self, patient_id):
        response = requests.get(f"{self.get_time_slot_url}/{patient_id}", headers=headers)
        return (response.json())
    
    def schedule_notif(self, patient_id, message, launch_datetime, coach_personality):
        request_body = {"MESSAGE": message,
                        "LAUNCH_DATETIME" : launch_datetime,
                        "COACH_PERSONALITY": coach_personality}
        print(request_body)
        print("posting")
        requests.post(f"{self.schedule_notif_url}/{patient_id}", json=request_body)


